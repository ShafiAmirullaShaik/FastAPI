...
def update_patient(patient_id: str, patient: PatientUpdate):
    try:
        ...
        patient_pydantic_object = Patient(id=patient_id, **existing_patient)
        validated_patient_info = patient_pydantic_object.model_dump(exclude={"id"})
        
        patients[patient_id] = validated_patient_info
            
        with open("patients.json", "w") as f:
            json.dump(patients, f)
            
        response = {"message": "Patient updated successfully", "patient": validated_patient_info}
